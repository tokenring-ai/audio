...
describe("AudioService", () => {
  ...
  beforeEach(() => {
    ...
    // Mock speech model
    mockSpeechModel = {
      generateSpeech: mock().mockResolvedValue([Buffer.from("test audio data")]),
    };
  ...
  it("should have correct name and description", () => {
    ...
    expect(audioService.description).toBe("Audio recording, playback, speech, and transcription backed by the shared media library");
  });
  ...
  it("should throw error when no provider is set", () => {
    ...
    expect(() => audioService.requireAudioProvider(agentWithoutProvider)).toThrow();
  });
